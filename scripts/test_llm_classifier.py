...
async def test():
    ...
    if bridge.qwen_enabled:
        result = await bridge.infer_intent("My name is Celestine.", {"target_ir": "WORKSPACE_QUERY", "confidence": 0.3})
        print(f"Result: {result}")
    else:
        print("Qwen not enabled")
