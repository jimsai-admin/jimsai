...
- **M-TRAIN — de-LLM autonomous training agent** (`prototype/jimsai/autonomous_training_agent.py`): the shape is right — a continuous **measure-gap → ingest-from-source → re-measure** loop over base-model coverage (language/domain/capability gaps) with a shared **base model** + per-workspace **personalization adapter** (`workspaces.py`, both in code). But it is **not yet de-LLM** (a `synthetic_generation` source is `groq_generated`) and its ingestion is a **scaffold** — it logs SPPE-pair counts, not wired to the real base artifacts. The de-LLM rebuild is the exact pattern proven by hand in 2026-07-10's coverage work: gaps come from the harness / messy gauntlet; the targeting plan calls the real SOURCED ingestion (`broaden_lexicon.py` → Wikidata concepts, `fetch_common_words.py` → frequency lists, ELE construction discovery → real corpora, all provenance-stamped); re-measure and stop when the gap closes; drop the LLM source. Base model enriched from public sources + engineer review; per-user model stays the runtime learning loop (facts/corrections, ~0.02 ms). Judge = gap-closure on the MESSY gauntlet, not clean templates. The common-vocabulary fix (recall 0→4/5, gap-honesty preserved) is the first real base-model training step and the template for the rest.
