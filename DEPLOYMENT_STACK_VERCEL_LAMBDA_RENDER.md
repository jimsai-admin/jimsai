...
- Render worker: Celery training worker from `services/training-pipeline/`
...
The worker uses `autoDeployTrigger: commit`, so every commit to the linked branch triggers a worker deploy.

Render background workers do not support the free instance type in the current Blueprint spec, so `render.yaml` sets `plan: starter`.
